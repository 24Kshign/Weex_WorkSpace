...
public class AlertToast {
    ...
    private static void showBlack(String text, int duration) {
        ...
        if (sToast == null) {
            View layout = LayoutInflater.from(WXApplication.getInstance()).inflate(R.layout.toast_view, null);
            sToast = new Toast(WXApplication.getInstance());
            sToast.setGravity(Gravity.CENTER_VERTICAL | Gravity.CENTER_HORIZONTAL, 0, 0);
            sToast.setDuration(duration);
            sToast.setView(layout);
        }
        TextView tv = sToast.getView().findViewById(R.id.toast_alert_txt);
        tv.setText(text);
        sToast.show();
    }
